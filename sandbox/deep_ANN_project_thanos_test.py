...
iter_error = np.zeros(len(hidden_units))
EPOCHS = 10000
MINI_BATCH_SIZE = 100
#N_HIDDEN_NEURONS = 10
LEARNING_RATE = 0.01
...
def train():
    ...
    k=0
    for train_index, test_index in CV1.split(x_train):
    
        # extract training and test set for current CV fold
        Xr_train = x_train[train_index,:]
        yr_train = y_train[train_index]
        Xr_test = x_train[test_index,:]
        yr_test = y_train[test_index]
        ...
        for train_index, test_index in CV2.split(Xr_train):
            ...
            for epoch in range(EPOCHS):
                net.train()
                pred = net(early_Xtrain)
                loss = criterion(pred, early_Ytrain)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                net.eval()
                pred = net(early_Xtest)
                loss = criterion(pred, early_Ytest)  
            ...
            y_test_pred = net(Xin_test).detach().numpy()

            # Calculate error (RMSE)
            nn_error_val[j,i] = np.sqrt(np.mean((y_test_pred-yin_test)**2))
        ...
        #PUT HERE THE VALIDATION NETWORK+CALC OF NEW ERRORS
