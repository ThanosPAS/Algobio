...
hidden_units = np.arange(start = 1, stop = 33, step = 2)
...
def train(net, train_loader, valid_loader, K1, K2, epochs, optimizer='SGD', lr=0.01, momentum=0, l2_reg=0, use_cuda=False, n_patience=10, use_early_stopping=False, verbose=False):
    ...
    nn_error = np.zeros(K1)
    nn_error_val_tot = np.zeros(K1)
    # Init optimal optimal h
    ...
    for train_index, test_index in CV1.split(train_loader):
        ...
        for train_index, test_index in CV2.split(Xr_train):
            ...
            if use_early_stopping == True:
                x_train_erl, x_test_erl, y_train_erl, y_test_erl = train_test_split(Xin_train, yin_train, test_size=0.3, random_state=41)
            ...
            for epoch in range(epochs):
                ...
                if use_early_stopping == True:
                    for x_train, y_train in [x_train_erl,y_train_erl]:
                        pred = net(x_train)
                    ...
                    for x_train, y_train in [Xin_train,yin_train]:
                        pred = net(x_train)
                        loss = criterion(pred, y_train)
                        optimizer.zero_grad()
                        loss.backward()
                ...
                if use_early_stopping == True:
                    train_loss.append(running_train_loss / len(x_train_erl))
                else:
                    train_loss.append(running_train_loss / len(Xin_train))
                ...
                if use_early_stopping == True:
                    for x_valid, y_valid in [x_test_erl,y_test_erl]:
                        pred = net(x_valid)
                        loss = criterion(pred, y_valid)
                        running_valid_loss += loss.data
                else:
                    for x_valid, y_valid in [Xin_test,yin_test]:
                        pred = net(x_valid)
                        loss = criterion(pred, y_valid)
                        running_valid_loss += loss.data
                
                if use_early_stopping == True:
                    valid_loss.append(running_valid_loss / len(x_test_erl))
                else:
                    valid_loss.append(running_valid_loss / len(Xin_test))
                    
                    
                # Evaluation of the tuned net
                y_test_pred = net(x_valid).detach().numpy()
    
                # Calculate error (RMSE)
                nn_error_val[j,i] = np.sqrt(np.mean((y_test_pred-y_valid)**2))
            ...
            # Collect mean of min errors. Final nested validation error
            ...
            for epoch in range(epochs):
                ...
                running_train_loss = 0
                for x_train, y_train in [Xr_train,yr_train]:
                    pred = net(x_train)
                    loss = criterion(pred, y_train)
                    optimizer.zero_grad()
                    loss.backward()
                ...
                train_eval_loss.append(running_train_loss / len(Xr_train))
        
                # validation
                net.eval()
                
                running_valid_loss = 0
                for x_valid, y_valid in [Xr_test,yr_test]:
                    pred = net(x_valid)
                    loss = criterion(pred, y_valid)
                    running_valid_loss += loss.data
                
                eval_loss.append(running_valid_loss / len(Xr_test))

            

                if verbose:
                    print("Epoch {}: Train loss: {:.5f} Valid loss: {:.5f}".format(epoch+1, train_eval_loss[epoch], eval_loss[epoch]))
        
                if invoke(early_stopping, valid_loss[-1], net, implement=use_early_stopping):
                    net.load_state_dict(torch.load('checkpoint.pt'))
                    break
            
                 y_nn_test_pred = net(x_valid).detach().numpy()
                 #Evaluation error
                 nn_error[k] = np.sqrt(np.mean((y_nn_test_pred-y_valid)**2))
    ...
    return net, optimizer, train_loss, valid_loss, train_eval_loss, eval_loss
